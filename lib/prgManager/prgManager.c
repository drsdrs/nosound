...
uint8_t prgManager_compile( const char* prgName, int waitForProcess ){
  ...
  prgName =  prgName;
  file_copy( "src/templates/prg_wrapper.c", path_wrapper_get( prgName ) );
  file_copy( "src/templates/CMakeLists.txt", path_cmakelist_get( prgName ) );
...
void prgManager_list_get( char*** fileListPointer, uint16_t* fileListLengthPointer ){
  ...
  while( (entry=readdir(folder)) )  {
    ...
    if( entry->d_name[0]=='.' ){ continue; }    //    not a folder ...
    if( entry->d_type != 4 ){ continue; }    //    not a folder ...

    fileListTemp[files] = malloc( sizeof( entry->d_name ) * sizeof( fileListTemp[files] ) );
...
int prgManager_init( const char* startPrgName, const char* cleanup ){
  ...
  } else { printf("%s not found or PRG running. prgRunning: %i\n", startPrgName, prgRunning); }
  return 0;
}
