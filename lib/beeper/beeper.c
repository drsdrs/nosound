...
void makeFrequencys() {
    ...
    for (int oct = 0; oct < 8; oct++) {
        for (int note = 0; note < noteAmount; note++) {
            ...
            double noteFreq =
                4.4 * ((1 << oct) + (noteDivider[note] * (1 << oct)));
...
void Beeper_audio_write(snd_pcm_sframes_t len, uint8_t** audio_buffer) {
  ...
  for (int i = 0; i < len; i += 1) {
    ...
    if (out) { tmpBuffer[i] = silence - volume; } else { tmpBuffer[i] = silence; }
  ...
  while( total_time_us < args->length_us){
    switch ( args->type ) {
      case PITCH_MOD_LINEAR:
        ...
        if (transform_state >= 1) total_time_us = args->length_us; // break out of main while loop
        break;
      case PITCH_MOD_SINE:
        float sine_val = waves_sine12_get((uint16_t)(transform_state*0xfff));
        //printf("sine: %f\n", sine_val);
        chFreq[ args->channel ] =
          notePhase[args->note_start]*(1-sine_val) +
      ...
      case PITCH_MOD_RANDOM:
        ...
        chFreq[ args->channel ] =
          notePhase[args->note_start]*(1-rndTemp) +
...
void Beeper_note_fade(
  uint8_t channel, uint8_t note_start, uint8_t note_end,
