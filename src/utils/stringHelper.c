...
char *string_append( const char *str0, const char *str1 ) {
    if ( str0 == NULL || str1 == NULL ) {
        printf( "STRING_APPEND error %s - %s \n", str0, str1 );
        return NULL;
    }
    char *new_str;
...
char *string_join( char **str, const char *delimiters ) {
    ...
    while ( str [ i ] != NULL ) {
        joinedStr = (char *)realloc(
            joinedStr, strlen( joinedStr ) + strlen( str [ i ] ) + strlen( delimiters ) + 1 );
...
//     /* Add space for terminating null string so caller knows where the list of returned strings
//     ends. */ count++; result = malloc(sizeof(char*) * count);
//
//     if (result){
//         size_t idx  = 0;
