...
void tv_render() { SDL_RenderPresent(renderer); }
...
void tv_border(uint16_t border_x, uint16_t border_y, uint16_t border_width,
  ...
  for (size_t x = 1; x < border_width; x++) {
    for (size_t y = 0; y <= 1; y++) {
      ...
      tv_char(border_x + x * 8, border_y + y * border_height * 8, symbol,
              color);
    }
  }
  ...
  if (border_type) {
    tv_char(border_x + (border_width)*8, border_y - 8,
            ASCII_BORDER_ROUND_TOP_RIGHT, color);
  } else {
    tv_char(border_x + (border_width)*8, border_y, ASCII_BORDER_TOP_RIGHT,
            color);
  }
  ...
  tv_char(border_x + (border_width)*8, border_y + border_height * 8, symbol,
          color);
}
...
void initWindow(uint8_t scale) {
  ...
  window = SDL_CreateWindow("SDL pixels", SDL_WINDOWPOS_UNDEFINED,
