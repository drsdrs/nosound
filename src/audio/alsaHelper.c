...
static void alsa_write( int nframes, uint8_t *samples ) {
    //printf("%li\n", nframes);
    if(alsa_running_flag==true) snd_pcm_writei( pcm, samples, nframes );
    ...
    if ( snd_timer_read( handle, &tr, sizeof( tr ) ) == sizeof( tr ) ) {
        ...
        if ( ( frames_to_deliver = snd_pcm_avail_update( pcm ) ) <
            ...
            if ( frames_to_deliver == -EPIPE ) {
                ...
                //framesMust = 0;
                //Beeper_audio_write( audio_buffer_length, &alsa_audio_buffer );
                //alsa_write( audio_buffer_length, alsa_audio_buffer );
        ...
        frames_to_deliver = frames_to_deliver > audio_buffer_length ? audio_buffer_length : frames_to_deliver;
...
int alsa_setup() {
    alsa_audio_buffer = (uint8_t *)malloc( audio_buffer_length * sizeof( uint8_t ) );
    memset( alsa_audio_buffer, 127 /*silence*/, audio_buffer_length );
    ...
    if ( ( err = snd_pcm_sw_params_set_avail_min( pcm, sw_params, audio_buffer_length ) ) < 0 ) {
        fprintf( stderr, "cannot set minimum available count (%s)\n", snd_strerror( err ) );
        exit( 1 );
    }
    if ( ( err = snd_pcm_sw_params_set_start_threshold( pcm, sw_params, audio_buffer_length ) ) < 0 ) {
        fprintf( stderr, "cannot set start mode (%s)\n", snd_strerror( err ) );
        exit( 1 );
    }

    if ( ( err = snd_pcm_sw_params_set_stop_threshold( pcm, sw_params, audio_buffer_length ) ) < 0 ) {
        fprintf( stderr, "cannot set snd_pcm_sw_params_set_stop_threshold mode (%s)\n",
                 snd_strerror( err ) );
        exit( 1 );
    }

    if ( ( err = snd_pcm_sw_params_set_silence_threshold( pcm, sw_params, 0 ) ) < 0 ) {
    ...
    for ( size_t i = 0; i < 0x10; i++ ) alsa_write( 409, alsa_audio_buffer );    //   PRE-BUFFERING
